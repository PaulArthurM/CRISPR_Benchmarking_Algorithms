...
def RunSimulation(essential_genes_mean, non_essential_variance, essential_variance, num_essential_genes,
                  ...
                  num_control_guides, depth_factor, binding_efficiency, rep_num, token):
    ...
    df_data['sgRNA'] = sg_Count_Names
    df_data = df_data[['sgRNA', 'Gene', 0, 1, 2, 3, 4]]
    df_data.columns = ['sgRNA', 'Gene', 'T0_1', 'T0_2', 'F_1', 'F_2', 'Essentiality']
    checkZero = df_data.copy().loc[:,['T0_1', 'T0_2', 'F_1', 'F_2']]
    checkZero['sum'] = checkZero.sum(axis=1)
    df_data = df_data[checkZero.loc[:,'sum']>0]

    file_to_write = './counts/%s.tsv' % token
    print("Writting %s" % file_to_write)
...
@click.command()
@click.option('--essential_genes_mean', default=1.2, help='Mean of essentials genes.')
@click.option('--non_essential_variance', default=1, help='Variance of non-essentials genes.')
@click.option('--essential_variance', default=1, help='Variance of essensials genes.')
@click.option('--num_essential_genes', default=1, help='Number of essentials genes.')
@click.option('--num_nonessential_genes', default=1, help='Number of non-essentials genes.')
@click.option('--num_guides_per_gene', default=1, help='Number of guides per gene.')
@click.option('--guide_efficiency_mean', default=1, help='Guide efficiency mean.')
@click.option('--guide_efficiency_variance', default=1, help='Guide efficiency variance.')
@click.option('--num_control_guides', default=1, help='Number of control guides.')
@click.option('--depth_factor', default=1, help='Depth factor.')
@click.option('--binding_efficiency', default=1, help='Binding efficiency.')
@click.option('--rep_num', default=1, help='Number of repetition.')
@click.option('--token', default="token", help='Token.', required=True)
def main(essential_genes_mean, non_essential_variance, essential_variance, num_essential_genes, num_nonessential_genes, num_guides_per_gene, guide_efficiency_mean, guide_efficiency_variance, num_control_guides, depth_factor, binding_efficiency, rep_num, token):
    np.random.seed(0)
    RunSimulation(essential_genes_mean, non_essential_variance, essential_variance, num_essential_genes,
            num_nonessential_genes, num_guides_per_gene, guide_efficiency_mean, guide_efficiency_variance, num_control_guides, depth_factor, binding_efficiency, rep_num, token)
